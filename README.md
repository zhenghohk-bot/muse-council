...
报告输出至 `eval-results/<run-id>/report.md` 和 `report.json`。评测分为「MVP 发布门槛」与「高质量目标」，避免把可发布基线和长期优化目标混为一谈。v0.5 的问题定位与修复、结构性验证、裁判模型迁移对可比性的影响，以及仍未达成的目标，见 [质量评测与迭代记录](docs/evaluation-v0.5.md)。
